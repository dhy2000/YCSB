...
// We also use `net.rubyeye.xmemcached.MemcachedClient`; it is not imported
...
public class MemcachedClient extends DB {
  ...
  public static final String FAILURE_MODE_PROPERTY_DEFAULT = "true";

  public static final String CONNECTION_POOL_PROPERTY = "memcached.connectionPool";
  public static final String DEFAULT_CONNECTION_POOL = "4";
   ...
   *     SpyMemcached.
   */
  protected net.rubyeye.xmemcached.MemcachedClient memcachedClient() {
    return client;
  }
  ...
  protected net.rubyeye.xmemcached.MemcachedClient createMemcachedClient()
      throws Exception {
    ...
    //builder.setReadBufferSize(Integer.parseInt(
    //    getProperties().getProperty(READ_BUFFER_SIZE_PROPERTY,
    //                                DEFAULT_READ_BUFFER_SIZE)));

    builder.setOpTimeout(Integer.parseInt(
    ...
    builder.setFailureMode(Boolean.parseBoolean(
        getProperties().getProperty(FAILURE_MODE_PROPERTY, FAILURE_MODE_PROPERTY_DEFAULT)));

    builder.setConnectionPoolSize(Integer.parseInt(
        getProperties().getProperty(CONNECTION_POOL_PROPERTY, DEFAULT_CONNECTION_POOL)));
  ...
  public Status insert(
    ...
    key = createQualifiedKey(table, key);
    try {
      if(memcachedClient().add(key, objectExpirationTime, toJson(values))){
        return Status.OK;
      } else {
        return Status.ERROR;
      }
  ...
  public void cleanup() throws DBException {
    if (client != null) {
      try{
        ...
        logger.error("Error shutdown memcached");
      }
    }
  }
